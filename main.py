...
def parse_amount(value: str) -> int:
    """Parse economy amounts like 10000, 10k, 2.5m, 1b, and 10,000."""
    ...
    try:
        if raw[-1:] in multipliers:
            number = float(raw[:-1])
            amount = int(number * multipliers[raw[-1]])
        else:
            amount = int(raw)
    except (ValueError, OverflowError):
        raise ValueError('Invalid amount. Examples: `10000`, `10k`, `2.5m`.') from None
    if amount <= 0 or amount > 1_000_000_000:
        raise ValueError('Amount must be between 1 and 1,000,000,000.')
    return amount
...
async def account(uid: int):
    a = await bot.db.get_account(uid)
    if not a:
        raise ValueError('You do not have a FROXX account yet. Use `fx start` to create one and receive 10,000 🪙.')
    return a
...
class HelpPanel(discord.ui.View):
    ...
    async def coin_free(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._show(interaction, '🪙 COIN + FREE COMMANDS', [
            ...
            '`fx pay @user <amount>` — Send coins; amount supports `10k`, `2.5m`, etc.',
...
@bot.command(name='start', aliases=['st', 'register', 'account'])
async def start(ctx):
...
async def open(ctx, item: str):
    item = item.lower()
    if item not in {'mystery_box','rare_crate','legendary_crate'}:
        raise ValueError('Unknown crate.')
    # Crates are settled in one DB transaction: consume -> coin reward -> rare
    # item -> ledger entry. This prevents repeat/open race exploits.
    crate_cfg = {
        'mystery_box': {'low': 250, 'high': 1700, 'item_chance': 0.025, 'item_pool': ['iron_scrap','copper_wire','moon_shard']},
        'rare_crate': {'low': 6500, 'high': 12150, 'item_chance': 0.018, 'item_pool': ['moon_shard','emerald_chip','golden_key']},
        'legendary_crate': {'low': 15000, 'high': 75150, 'item_chance': 0.012, 'item_pool': ['emerald_chip','ruby_core','void_fragment']},
    }[item]
    coins = rng.randint(crate_cfg['low'], crate_cfg['high'])
    reward_item = None
    if rng.random() < crate_cfg['item_chance']:
        reward_item = rng.choice(crate_cfg['item_pool'])
    result = await bot.db.open_crate_atomic(ctx.author.id, item, coins, reward_item, rng.randint(20, 150), secrets.token_hex(16))
    parts = [f'**{item}** → **+{fmt(result["coins"])} {COIN}**']
    if result.get('item'):
        parts.append(f'🎁 BONUS ITEM: **{result["item"]}**')
    parts.append('🔒 One-time atomic settlement • no duplicate-open loop')
    await safe_send(ctx, embed=emb('🎁 CRATE CRACKED!', '\n'.join(parts), 0xF1C40F))
...
async def pay(ctx, member: discord.Member, amount: str):
    ...
    try:
        ...
        if str(exc) == 'account missing':
            raise ValueError(f'{member.mention} does not have a FROXX account yet. They must use `fx start` first.') from None
        raise
...
class MinesView(discord.ui.View):
    ...
    async def explode(self, interaction: discord.Interaction, mine: int) -> None:
        # IMPORTANT: the bet was debited when the round started. A mine hit is a
        # real settlement, not just a UI state change. Settling here closes the
        # pending game_round so the player can immediately start another game.
        self.active = False
        await game_payout(self.ctx.author.id, 0, 'mines_loss')
        for child in self.children:
            child.disabled = True
            if isinstance(child, MinesButton) and child.index == mine:
                child.label = '💣'
        self.cashout.disabled = True
        await interaction.response.edit_message(embed=emb('💥 MINES — BOOM', f'{self.board_text()}\n\n💣 You hit a mine. **-{fmt(self.bet)} {COIN}**\nMine count: **{self.mine_count}**\n\n✅ Round settled — you can play again now.', 0xED4245), view=self)
        self.stop()
...
async def admin(ctx):
    await safe_send(ctx, embed=emb('🛡️ ADMIN',
        ...
        '`fx admin remove @user amount` / `charge @user amount [reason]`\n'
...
@commands.check(is_admin)
async def admin_add(ctx, member: discord.Member, amount: str):
    amount = parse_amount(amount)
...
@commands.check(is_admin)
async def admin_remove(ctx, member: discord.Member, amount: str):
    amount = parse_amount(amount)
...
@admin.command(name='charge', aliases=['deduct','fine'])
@commands.check(is_admin)
async def admin_charge(ctx, member: discord.Member, amount: str, *, reason: str = 'Admin charge'):
    amount = parse_amount(amount)
    a = await account(member.id)
    if a.wallet < amount:
        raise ValueError('User does not have enough wallet coins.')
    await bot.db.economy_tx(
        member.id,
        'ADMIN CHARGE',
        wallet_delta=-amount,
        meta={'admin_id': ctx.author.id, 'reason': reason[:300]},
        tx_id=secrets.token_hex(16),
    )
    await safe_send(ctx, embed=emb('💀 ADMIN CHARGE', f'{member.mention} was charged **-{fmt(amount)} {COIN}**.\nReason: **{discord.utils.escape_markdown(reason[:300])}**\n\n📜 The charge is permanently recorded in `fx transactions`.', 0xED4245))

@admin.command(name='set')
@commands.check(is_admin)
async def admin_set(ctx, member: discord.Member, amount: str):
    amount = parse_amount(amount)
