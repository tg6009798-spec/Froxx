...
class Database:
    ...
    async def charge_game(self, user_id: int, bet: int, key: str, cooldown: int = 1, tx_id: str | None = None) -> Account:
        ...
        async with self.tx() as db:
            ...
            # A player may only have one unsettled game round at a time.
            # This prevents a second round from accidentally settling the first
            # one after a rapid command sequence or a delayed interaction.
            pending = await (await db.execute(
                "SELECT 1 FROM game_rounds WHERE user_id=? AND status='pending' LIMIT 1",
                (user_id,),
            )).fetchone()
            if pending:
                raise ValueError("game already active")
            if row["wallet"] < bet:
    ...
    async def open_crate_atomic(
        ...
        """Open one crate atomically and record the complete outcome.

        The crate is consumed and every reward is committed in the same SQLite
        transaction. A safety cap also prevents a crate+item combination from
        exceeding the crate purchase price by more than 150 coins.
        """
        if coins < 0 or xp < 0:
            raise ValueError("invalid crate reward")
        async with self.tx() as db:
            user = await (await db.execute("SELECT * FROM users WHERE user_id=?", (user_id,))).fetchone()
            crate = await (await db.execute("SELECT * FROM items WHERE item_id=? AND active=1 AND item_type='crate'", (crate_id,))).fetchone()
            if not user or not crate:
                raise ValueError("account or crate missing")
            if await (await db.execute("SELECT 1 FROM transactions WHERE tx_id=?", (tx_id,))).fetchone():
                return {"coins": 0, "item": None}
            inv = await (await db.execute("SELECT quantity FROM inventory WHERE user_id=? AND item_id=?", (user_id, crate_id))).fetchone()
            if not inv or inv[0] < 1:
                raise ValueError("You do not own that crate.")

            item_row = None
            if reward_item:
                item_row = await (await db.execute("SELECT * FROM items WHERE item_id=? AND active=1", (reward_item,))).fetchone()
                if not item_row:
                    reward_item = None

            # Economy guard: a crate may lose money, break even, or make at most
            # +150 coins when a bonus item is included. Mystery Box cash is also
            # bounded by the public 1,700-coin maximum in main.py.
            item_value = int(item_row["sell_price"]) if item_row else 0
            max_total = int(crate["buy_price"]) + 150
            coins = min(int(coins), max(0, max_total - item_value))

            new_qty = inv[0] - 1
            if new_qty:
                await db.execute("UPDATE inventory SET quantity=? WHERE user_id=? AND item_id=?", (new_qty, user_id, crate_id))
            else:
                await db.execute("DELETE FROM inventory WHERE user_id=? AND item_id=?", (user_id, crate_id))
            ...
            ts = now_ts()
            await db.execute(
                "UPDATE users SET wallet=wallet+?, xp=max(0,xp+?), total_earned=total_earned+?, last_active=? WHERE user_id=?",
                (coins, xp, coins, ts, user_id),
            )
            await db.execute(
                "INSERT INTO transactions(tx_id,user_id,kind,amount,wallet_delta,meta_json,created_at) VALUES(?,?,?,?,?,?,?)",
                (tx_id, user_id, "Crate Open", coins, coins, json.dumps({"crate": crate_id, "coins": coins, "item": reward_item, "item_sell_value": item_value, "xp": xp}, separators=(",", ":")), ts),
            )
            return {"coins": coins, "item": reward_item}
    ...
    async def reset_user(self, user_id: int) -> None:
        """Reset one economy account without leaving orphaned state behind."""
        async with self.tx() as db:
            # Explicit cleanup keeps this safe even on databases created by an
            # older schema where every foreign-key relationship was not present.
            await db.execute("DELETE FROM trade_sessions WHERE user_a=? OR user_b=?", (user_id, user_id))
            ...
            await db.execute("DELETE FROM game_rounds WHERE user_id=?", (user_id,))
